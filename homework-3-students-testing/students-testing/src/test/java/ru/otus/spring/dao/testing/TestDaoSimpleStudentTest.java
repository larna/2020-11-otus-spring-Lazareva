...
class TestDaoSimpleStudentTest {
    ...
    void setUp() {
        ...
        Person student = new Person("Ivanov", "Ivan", 19);
    ...
    void shouldHaveCorrectFindAll() {
        //Ivanov уже сохранен
        Person petrov = new Person("Petrov", "Peter", 19);
        StudentTest petrovTest = new StudentTest(LocalDate.now(), petrov, new ArrayList<>());
        dao.save(petrovTest);

        Person ivanov = new Person("Ivanov", "Ivan", 19);
    ...
    void findByStudentAndDate() {
        Person searchStudent = new Person("Ivanov", "Ivan", 19);
        StudentTest test = dao.findByStudentAndDate(searchStudent, LocalDate.now());
        assertAll(() -> assertNotNull(test),
                () -> assertEquals("Ivanov", test.getStudent().getSurname()),
                () -> assertEquals("Ivan", test.getStudent().getName()),
    ...
    void shouldHaveTextNotFoundExceptionIfTestNoFound() {
        Person searchStudent = new Person("Petrov", "Peter", 19);
