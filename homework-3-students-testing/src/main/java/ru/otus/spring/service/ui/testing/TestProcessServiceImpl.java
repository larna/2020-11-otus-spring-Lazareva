...
public class TestProcessServiceImpl implements TestProcessService {
    ...
    private static final String COMEBACK = "testProcess_comeback";
    private static final String TEST_ALREADY_EXISTS = "testProcess_alreadyExists";
    private static final String ERROR = "testProcess_error";
    ...
    public TestResultsReport testStudent(Person student) {
        ...
        try {
            ...
            String alreadyExistsMessage = localizationService.getMessage(TEST_ALREADY_EXISTS, student.getFullName());
            logger.info(alreadyExistsMessage);
            test = testService.getTestByStudentAndDate(student, LocalDate.now());
            String comeBackTomorrowMessage = localizationService.getMessage(COMEBACK);
    ...
    private void askQuestions(StudentTest test) {
        ...
        for (Question question : test.getQuestions()) {
            try {
                ...
                ioService.outMessage(localizationService.getMessage(ERROR));
            }
        }
    }
}
