...
public class AuthorsViewConsoleTable implements View<Author> {
    ...
    private static final String[] TABLE_HEADER_ROW = new String[]{"Номер", "Имя", "Настоящее имя, если автор издается под псевдонимом", "Дата рождения"};
    ...
    private AsciiTable getAuthorsTable(List<Author> authors) {
        ...
        IntStream.range(0, authors.size()).forEach(i -> {
            table.addRow(authorToRow(authors.get(i), i+1));
            table.addRule();
        });
     ...
     * @return
     */
    private String[] authorToRow(Author author, Integer number) {
