...
public class CommentsViewConsoleTable implements View<Comment> {
     ...
     * Заголовок таблицы
     */
    private static final String[] TABLE_HEADER_ROW = new String[]{"Номер", "Текст", "Книга"};
    ...
    private AsciiTable getCommentsTable(List<Comment> comments) {
        ...
        IntStream.range(0, comments.size()).forEach(i -> {
            table.addRow(commentToRow(comments.get(i), i + 1));
            table.addRule();
        });
        table.getRenderer().setCWC(new CWC_LongestWordMin(new int[]{-1, 30, 70}));
        return table;
     ...
     * @return
     */
    private String[] commentToRow(Comment comment, Integer number) {
        String idCell = number.toString();
        String text = comment.getDescription();
        String book = comment.getBook().getName() + " (ID = " + comment.getBook().getId() + ")";
