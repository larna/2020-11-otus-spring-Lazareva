...
public class BookUpdateHandler {
    ...
    public void onApplicationEvent(BookEvent event) {
        Thread.sleep(100);
        try {
            Book updatingBook = event.getBook();
            refreshAuthors(updatingBook);
            refreshGenre(updatingBook);
            bookService.save(updatingBook);
            System.out.println(booksView.getObjectView(updatingBook, "Книга успешно изменена"));
        } catch (Exception e) {
